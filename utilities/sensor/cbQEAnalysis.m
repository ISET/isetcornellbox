...
m = pinv(rgbMeanSimPad) * rgbMeanMeas; 
ieNewGraphWin;
plot(rgbMeanSimPad * m, rgbMeanMeas, 'o');


qe = [sensorGet(thisSensorSim, 'spectral qe')];
illuA = ieReadSpectra(lightNameA, wave);
illuCWF = ieReadSpectra(lightNameCWF, wave);
illuDay = ieReadSpectra(lightNameDay, wave);
illuARep = repmat(illuA, [1,24]);
illuCWFRep = repmat(illuCWF, [1, 24]);
illuDayRep = repmat(illuDay, [1, 24]);
refl = ieReadSpectra('MiniatureMacbethChart.mat',wave);
simA = illuARep.*refl;
simCWF = illuCWFRep.*refl;
simDay = illuDayRep.*refl;
simRad = [simA,simDay,simCWF];
tmp = [qe' *simRad;ones(1, 72)];

m2 = pinv(tmp') * rgbMeanMeas;
est = tmp' * m2;
ieNewGraphWin;
hold all;
plot(rgbMeanMeas(:,1), est(:,1), 'ro');
plot(rgbMeanMeas(:,2), est(:,2), 'go');
plot(rgbMeanMeas(:,3), est(:,3), 'bo');
