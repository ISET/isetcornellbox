...
%}
%{
tmp = load('p4aLensVignetIlluA.mat', 'pixel4aLensVignetSlopeIlluAR');
vignetting = tmp.pixel4aLensVignetSlopeIlluAR;
%}
%{
ieNewGraphWin;
imagesc(vignetting);
...
legend('R-nominal', 'G-nominal', 'B-nominal',...
       'R-corrected', 'G-corrected', 'B-corrected');
%}
%% Save results
...
plot(wave, spdDay); 
xlabel('Wavelength (nm)'); ylabel('Radiance (Watt/sr/nm/m^2)');
title('Illuminant Day'); xlim([400 700]); box on; grid on;